...
@section('content')
<div class="container">
    <h1>Ajouter une nouvelle Image Lookbook</h1>

    <form action="{{ route('admin.image-lookbooks.store') }}" method="POST">
        @csrf

        <div class="mb-3">
            <label for="url" class="form-label">ID Google Drive (de l'image)</label>
            <input type="text" id="url" name="url" class="form-control" value="{{ old('url') }}" placeholder="Ex : 1u-4cc96XySUagKeV4bzIF_HQ5HB1yJ8G" required>
            @error('url')
        ...
        <div class="mb-3">
            ...
            <select id="lookbook_id" name="lookbook_id" class="form-select" required>
                ...
                @foreach($lookbooks as $lookbook)
                    <option value="{{ $lookbook->id }}" {{ old('lookbook_id') == $lookbook->id ? 'selected' : '' }}>
                        {{ $lookbook->titre }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3 form-check">
            <input type="checkbox" id="is_principale" name="is_principale" class="form-check-input" {{ old('is_principale') ? 'checked' : '' }}>
