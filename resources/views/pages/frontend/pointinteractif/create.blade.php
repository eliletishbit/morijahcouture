...
@section('content')
<div class="container">
    <h1>Créer un nouveau Point Interactif</h1>

    <form action="{{ route('admin.point-interactifs.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-3">
            ...
            <select id="image_lookbook_id" name="image_lookbook_id" class="form-select" required>
                ...
                @foreach($images as $img)
                    <option value="{{ $img->id }}" {{ old('image_lookbook_id') == $img->id ? 'selected' : '' }}>{{ $img->url }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select id="produit_id" name="produit_id" class="form-select">
                ...
                @foreach($produits as $prod)
                    <option value="{{ $prod->id }}" {{ old('produit_id') == $prod->id ? 'selected' : '' }}>{{ $prod->nom }}</option>
                @endforeach
            </select>
            @error('produit_id')
                <div class="text-danger">{{ $message }}</div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" min="0" max="1" id="position_x" name="position_x" class="form-control" value="{{ old('position_x') }}" required>
            @error('position_x')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" min="0" max="1" id="position_y" name="position_y" class="form-control" value="{{ old('position_y') }}" required>
            @error('position_y')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea id="description_popup" name="description_popup" class="form-control">{{ old('description_popup') }}</textarea>
