...
<div class="container">
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" id="productFormEdit">
        ...
        <div class="mb-3">
            ...
            <select name="sous_categorie_id" class="form-select" required id="sousCategorieSelectEdit">
                @foreach ($sousCategories as $sousCategorie)
                <option value="{{ $sousCategorie->id }}" @selected(old('sous_categorie_id', $product->sous_categorie_id) == $sousCategorie->id) data-name="{{ strtolower($sousCategorie->nom) }}">{{ $sousCategorie->nom }}</option>
                @endforeach
            </select>
        </div>
        ...
        <!-- Bloc dynamique des pièces -->
        <div id="piecesSectionEdit" style="display:none;">
            <h4>Modifier les pièces (jusqu'à 7)</h4>
            <div id="piecesContainerEdit">
                @if(old('pieces'))
                    @foreach(old('pieces') as $index => $oldPieceId)
                        <div class="mb-3">
                            <label class="form-label">Pièce #{{ $index + 1 }}</label>
                            <input type="number" name="pieces[]" class="form-control" placeholder="ID du produit pièce" min="1" value="{{ $oldPieceId }}" required />
                ...
                @else
                    @foreach($product->pieces as $index => $piece)
                        <div class="mb-3">
                            <label class="form-label">Pièce #{{ $index + 1 }}</label>
                            <input type="number" name="pieces[]" class="form-control" placeholder="ID du produit pièce" min="1" value="{{ $piece->id }}" required />
                            <button type="button" class="btn btn-danger btn-sm mt-1 remove-piece-btn">Supprimer</button>
                        </div>
                    @endforeach
                @endif
            </div>
            <button type="button" id="addPieceBtnEdit" class="btn btn-secondary">+ Ajouter une pièce</button>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const sousCategorieSelect = document.getElementById('sousCategorieSelectEdit');
        const piecesSection = document.getElementById('piecesSectionEdit');
        const piecesContainer = document.getElementById('piecesContainerEdit');
        const addPieceBtn = document.getElementById('addPieceBtnEdit');
        let piecesCount = piecesContainer.children.length;
        const maxPieces = 7;
        ...
        addPieceBtn.addEventListener('click', function () {
            ...
            pieceField.innerHTML = `
                <label class="form-label">Pièce #${piecesCount}</label>
        ...
        piezasContainer.addEventListener('click', function(e){
